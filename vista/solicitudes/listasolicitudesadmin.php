        ...
        <?php while ($solicitudes = mysqli_fetch_array($query)) { ?>
            <tr>
                ...
                <td>
                    ...
                    <?php if ($solicitudes['estado'] == 1 && $solicitudes['tiposolicitud'] == 1) { ?> <button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#factura" onclick="detallefactura('<?php echo $solicitudes['idsolicitud']?>')"><i class="fa-solid fa-check-to-slot fa-xl"></i></button><?php } ?>
                </td>
            </tr>
        <?php } ?>
        </tbody>
